...
import com.gura.step09camera.R;
...
public class MainActivity extends AppCompatActivity
    ...
    public void requestTakePic(){
        ...
        if(intent.resolveActivity(getPackageManager())!=null){ //intent를 받아줄 수있는 액티비티가 있는지 확인
            //이미지를 저장할 파일 객체를 얻어온다.
            File photoFile=null;
            try{ //파일을 하나 만들고 이곳에 저장해달라고 요청
                //파일명 지정
                String timeStamp=
            ...
            if(photoFile!=null){
                //사진을 저장할 파일의 Uri 객체를 얻어온다.           파일프로바이더 , 포토파일
    ...
    public static void fitToImageView(ImageView imageView, String absolutePath){
        ...
        Bitmap bitmap = BitmapFactory.decodeFile(absolutePath, bmOptions); //비트맵을만든다.
